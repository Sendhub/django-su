...
class UserSuForm(forms.Form):
    ...
    user = forms.ModelChoiceField(label=_("Users"), queryset=User._default_manager.order_by(username_field), required=True)  # pylint: disable=W0212
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "ajax_select" in settings.INSTALLED_APPS and getattr(settings, "AJAX_LOOKUP_CHANNELS", None):  # pragma: no cover
    ...
    def __str__(self):
        if "formadmin" in settings.INSTALLED_APPS:  # pragma: no cover
            try:
